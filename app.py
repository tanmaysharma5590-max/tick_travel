...
# Load environment variables BEFORE importing `main`, since main.py may read
# os.getenv() at module import time. Two sources are supported:
#   1. Local development -> variables from a .env file
#   2. Streamlit Community Cloud -> variables set in the app's Secrets panel
#      (Settings -> Secrets), exposed via st.secrets, copied into os.environ
#      so that plain os.getenv() calls in main.py still work.
load_dotenv()

REQUIRED_KEYS = ["GROQ_API_KEY", "AVIATIONSTACK_API_KEY", "TAVILY_API_KEY", "DATABASE_URL"]
